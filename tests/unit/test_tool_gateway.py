...
def test_gateway_validation():
    ...
    with pytest.raises(ToolGatewayError, match="validation_error"):
        gateway.call(
            tool_name="get_inventory",
            payload={"sku": "MAT-001", "location_id": 123},
            execution_id="exec_1",
            state=ExecutionState.RECEIVED,
        )
...
def test_gateway_budget():
    ...
    gateway.call(
        tool_name="get_inventory",
        payload={"sku": "A", "location_id": "loc"},
        execution_id="exec_1",
        state=ExecutionState.RECEIVED,
    )
    gateway.call(
        tool_name="get_inventory",
        payload={"sku": "B", "location_id": "loc"},
        execution_id="exec_1",
        state=ExecutionState.RECEIVED,
    )
    ...
    gateway2.call(
        ...
        payload={"sku": "MAT-001", "quantity": 10},
        execution_id="exec_1",
    ...
    with pytest.raises(ToolGatewayError, match="budget_exceeded"):
        gateway2.call(
            tool_name="search_suppliers",
            payload={"sku": "MAT-002", "quantity": 10},
            execution_id="exec_1",
            state=ExecutionState.CONTEXT_LOADED,
        )
...
def test_gateway_idempotency():
    ...
    res3 = gateway.call(
        tool_name="get_inventory",
        payload={"sku": "MAT-002", "location_id": "warehouse_north"},
        execution_id="exec_1",
        state=ExecutionState.RECEIVED,
    )
