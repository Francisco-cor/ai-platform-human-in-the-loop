...
def test_calculate_shortage_missing_forecast():
    ctx = load_context_from_fixtures(
        inventory_fixture={
            "location_id": "warehouse_north",
            "items": [{"sku": "MAT-001", "on_hand": 100, "reserved": 0, "in_transit": 0}],
        },
        open_orders_fixture=[],
    )
    # no forecast
    ...
    assert (
        res.shortage_qty == 0
    )  # 20 -100 => 0? Wait: available 100, requested 20 => shortage 0 (since no forecast, shortage = max(0, requested - total) =0)
...
def test_detect_duplicate_open_order():
    ...
    dup2 = detect_duplicate_open_order(
        sku="MAT-001",
        location_id="warehouse_north",
        quantity=105,
        unit="piece",
    ...
    dup3 = detect_duplicate_open_order(
        sku="MAT-001",
        location_id="warehouse_north",
        quantity=100,
        unit="piece",
    ...
    dup4 = detect_duplicate_open_order(
        sku="MAT-001",
        location_id="warehouse_north",
        quantity=200,
        unit="piece",
...
def test_calculate_shortages_multiple_items_and_determinism():
    ...
    items = [
        {"sku": "MAT-001", "quantity": 120, "unit": "piece"},
        {"sku": "MAT-002", "quantity": 10, "unit": "piece"},
    ]
    first = calculate_shortages(
        items=items, location_id="warehouse_north", horizon_days=21, ctx=ctx
    )
    second = calculate_shortages(
        items=items, location_id="warehouse_north", horizon_days=21, ctx=ctx
    )
