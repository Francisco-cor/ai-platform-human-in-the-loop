...
def test_valid_transitions():
    ...
    assert is_valid_transition(
        ExecutionState.NORMALIZED, ExecutionState.BLOCKED
    )  # terminal branch always valid
...
def test_scope_hash_deterministic():
    ...
    h1 = Proposal.compute_scope_hash(
        proposal_id="prop_1", supplier_id="sup_1", lines=lines, total=50.0, currency="USD"
    )
    h2 = Proposal.compute_scope_hash(
        proposal_id="prop_1", supplier_id="sup_1", lines=lines, total=50.0, currency="USD"
    )
    assert h1 == h2
    assert h1.startswith("sha256:")
    h3 = Proposal.compute_scope_hash(
        proposal_id="prop_1", supplier_id="sup_1", lines=lines, total=51.0, currency="USD"
    )
    assert h1 != h3
