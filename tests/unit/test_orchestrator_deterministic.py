...
def test_deterministic_proposal_same_fixtures_same_result():
    ...
    orch1 = WorkflowOrchestrator(
        inventory_context=ctx, supplier_catalog=catalog, policy_config=policy
    )
...
def test_proposal_with_missing_forecast():
    ctx = load_context_from_fixtures(
        inventory_fixture={
            "location_id": "warehouse_north",
            "items": [{"sku": "MAT-999", "on_hand": 5, "reserved": 0, "in_transit": 0}],
        },
        open_orders_fixture=[],
    )
    catalog = load_catalog_from_fixtures(
        {
            ...
            "quotes": [{"sku": "MAT-999", "unit_price": 10.0}],
        }
    )
    policy = PolicyConfig(budget_limits={("tenant_demo", "*"): 5000})
    orch = WorkflowOrchestrator(
    ...
    norm = NormalizedRequest(
        ...
        tenant_id="tenant_demo",
        requester_id="user_01",
        items=[{"sku": "MAT-999", "quantity": 20, "unit": "piece"}],
        horizon_days=21,
        location_id="warehouse_north",
        currency="USD",
    )
...
def test_unit_conversion_in_proposal():
    ...
    orch = WorkflowOrchestrator(
        ...
        policy_config=PolicyConfig(budget_limits={("tenant_demo", "*"): 5000}),
    )
    SessionLocal = get_sessionmaker()
    db = SessionLocal()
    ...
    norm = NormalizedRequest(
        ...
        tenant_id="tenant_demo",
        requester_id="user_01",
        items=[{"sku": "MAT-001", "quantity": 1, "unit": "box"}],
        horizon_days=10,
        location_id="warehouse_north",
        currency="USD",
    )
    ...
    assert (
        exec_obj.proposal.lines[0].quantity == pytest.approx(8.0, rel=1e-3)
        or exec_obj.proposal.lines[0].quantity == 8
    )  # shortage 8 boxes, but qty max(shortage, requested)=8
    db.close()
