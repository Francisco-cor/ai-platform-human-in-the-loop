...
def _jwt(tenant="tenant_demo", roles=None):
    ...
    payload = base64.urlsafe_b64encode(json.dumps({"sub": "u1", "tenant_id": tenant, "roles": roles}).encode()).decode().rstrip("=")
...
def test_approver_required_success(client: TestClient):
    # create execution
    resp = client.post("/v1/procurement/executions", json={"tenant_id": "tenant_demo", "raw_intent": "rbac test"})
    assert resp.status_code == 202
    appr_id = resp.json()["approval_request"]["approval_id"]
    token = _jwt(tenant="tenant_demo", roles=["approver"])
    r = client.post(f"/v1/approvals/{appr_id}/decision", json={"decision": "approved", "decided_by": "approver_01"}, headers={"Authorization": f"Bearer {token}"})
    assert r.status_code == 200
    assert r.json()["status"] in ("approved", "partially_approved")


def test_requester_forbidden_on_approval(client: TestClient):
    resp = client.post("/v1/procurement/executions", json={"tenant_id": "tenant_demo", "raw_intent": "rbac forbidden"})
    appr_id = resp.json()["approval_request"]["approval_id"]
    token = _jwt(tenant="tenant_demo", roles=["requester"])
    r = client.post(f"/v1/approvals/{appr_id}/decision", json={"decision": "approved", "decided_by": "user_requester"}, headers={"Authorization": f"Bearer {token}"})
    assert r.status_code == 403
    assert r.json()["code"] == "forbidden"


def test_tenant_mismatch_forbidden_on_approval(client: TestClient):
    resp = client.post("/v1/procurement/executions", json={"tenant_id": "tenant_demo", "raw_intent": "tenant mismatch"})
    appr_id = resp.json()["approval_request"]["approval_id"]
    token = _jwt(tenant="tenant_other", roles=["approver"])
    r = client.post(f"/v1/approvals/{appr_id}/decision", json={"decision": "approved", "decided_by": "approver_other"}, headers={"Authorization": f"Bearer {token}"})
    assert r.status_code == 403
